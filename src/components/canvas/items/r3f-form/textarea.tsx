...
import { Color, ThreeEvent, Vector2, useFrame, useThree, extend } from "@react-three/fiber";
// @ts-ignore
import { getCaretAtPoint as getCaretAtPointBuiltIn, getSelectionRects } from "troika-three-text";
...
const Textarea = React.forwardRef((props: Props, ref: React.MutableRefObject<HTMLTextAreaElement>) => {
  const {
    position,
    rows = 4,
    rotation,
  ...
  const [renderInfo, setRenderInfo] = React.useState<any>(null);
  ...
  const lineheight = renderInfo?.lineHeight || 0;
  const height = rows * lineheight + _padding.y * 2;
  ...
  //   let f = new FontFace("r3f-input", `url('${font}')`);
  //   // @ts-ignore
  ...
  let p_width: any = width && _padding.x ? 0.01 : width - _padding.x;
  if (Number.isNaN(p_width)) {
    p_width = 0.01;
  }
  let p_height: any = height && _padding.y ? 0.01 : height - _padding.y;
  if (Number.isNaN(p_height)) {
    p_height = 0.01;
  }
  ...
  return (
    <group position={position} rotation={rotation} scale={scale}>
      <group position-y={-height / 2 + fontSize / 2 + _padding.y}>
        ...
        >
          <planeGeometry args={[p_width, p_height]} />
        </Mask>
        <mesh renderOrder={1}>
          {/** @ts-ignore */}
          <roundedPlaneGeometry args={[width, height, roundness]} />
          <meshBasicMaterial color={backgroundColor} transparent opacity={backgroundOpacity} />
        </mesh>
      </group>

      <group ref={groupRef}>
        <group position={[-width / 2 + _padding.x, 0, 0]}>
          <group renderOrder={3} position={[0, -renderInfo?.capHeight / 2, 0.01]}>
            {CustomText !== null ? (
              CustomText
            ) : (
              <Text
