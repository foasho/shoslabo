...
vec4 applyWind(vec4 v){
  float boundedYNormal=remap(normal.y,-1.,1.,0.,1.);
  float posXZ=position.x+position.z;
  float power=u_windSpeed/5.*-.5;
  
  float topFacing=remap(sin(u_windTime+posXZ),-1.,1.,0.,power);
  float bottomFacing=remap(cos(u_windTime+posXZ),-1.,1.,0.,.05);
...
void main(){
  ...
  vec3 inflatedVertexOffset=inflateOffset(vec3(vertexOffset,0.));
  
  vec4 worldViewPosition=modelViewMatrix*vec4(position,1.);
  
  worldViewPosition+=vec4(mix(vec3(0.),inflatedVertexOffset,u_effectBlend),0.);
