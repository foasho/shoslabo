...
const getAllBlogs = async (): Promise<any[]> => {
  const _blogs = await fetch(`${process.env.NEXT_PUBLIC_SERVICE_URL}/api/blog/list?all=true`, {
    cache: "no-store",
  }).then((res) => {
    if (res.status === 200) {
      return res.json();
...
export default async function BlogPage() {
  ...
  return (
    <div className='fixed left-0 top-0 z-10 h-screen w-screen overflow-y-auto'>
      <Header fontColor={"#1e1e1e"} withLogin />
      <div className='container mx-auto px-4 py-8 pt-24 text-gray-800'>
        {/** タイトル */}
        <div className='mb-8 text-4xl font-bold'>Blog</div>
        {/** サブタイトル */}
        <div className='mb-8 text-xl font-bold'>ここには、技術的なメモを残していきます。</div>
        {/** カードリスト */}
        <div className='-mx-4 flex flex-wrap'>
          {blogs.map((card, idx) => (
            <Link key={`card-${idx}`} href={`/blogs/${card.id}/edit`} className='mb-8 w-full px-4 md:w-1/2 lg:w-1/3'>
